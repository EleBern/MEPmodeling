...
def load_muap(plotOn=0, amplitudeDist=0):
    ...
    h5_path = os.path.join(root, "data_MUAP", "muap5.h5")
...
def amplitude_distribution(muaps):
    ...
    plt.title("MUAPs amplitude distribution - hand 4")
    plt.text(1, 2, "a={0}".format(np.round(popt[0], 4)))
    plt.text(1, 1.8, "b={0}".format(np.round(popt[1], 0)))
