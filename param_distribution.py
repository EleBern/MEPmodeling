...
# Delay against lambda
def straight_line(x, a, b):
    return a * x + b
popt, pcov = curve_fit(straight_line, total_lam, total_axonalDelay, p0=[2.5, 0])
perr = np.sqrt(np.diag(pcov)) * 10

fig = plt.figure()
plt.plot(lam[1], axonalDelay[1], "*r", label="1")
plt.plot(lam[2], axonalDelay[2], "*k", label="2")
plt.plot(lam[3], axonalDelay[3], "*g", label="3")
plt.plot(lam[4], axonalDelay[4], "*b", label="4")
plt.plot(lam[5], axonalDelay[5], "*m", label="5")
plt.plot([min(total_lam), max(total_lam)], straight_line(np.array([min(total_lam), max(total_lam)]), *(popt+perr)), "--k")
plt.plot([min(total_lam), max(total_lam)], straight_line(np.array([min(total_lam), max(total_lam)]), *(popt-perr)), "--k")
plt.plot([min(total_lam), max(total_lam)], straight_line(np.array([min(total_lam), max(total_lam)]), *popt), "--k", label="Best fit")
plt.ylabel("Delay [ms]")
plt.xlabel("Lambda [ms]")
plt.title("Delay against lambda")
plt.legend()
plt.show()

print("delay = {0} * lam + {1}".format(np.round(popt[0], 2), np.round(popt[1], 2)))
print("The standard deviation of the best fit parameters is {0} for the coefficient, {1} for the offset".format(np.round(perr[0], 2), np.round(perr[1], 2)))